  ...
  return (
    ...
    >
      ...
      <motion.div variants={container} className="relative container mx-auto px-4 text-center">
        ...
        <motion.div variants={container} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-16">
          {steps.map((step, index) => (
            <motion.div
              variants={item}
              key={index}
              className="group bg-[#222529] p-8 py-12 rounded-lg flex flex-col items-center transform hover:scale-105 hover:bg-green-600 transition-all duration-300"
            >
              <div className="mb-6 text-white">{step.icon}</div>
              <h3 className="text-2xl font-semibold text-green-500 group-hover:text-white transition-colors duration-300 mb-3">{step.title}</h3>
              <p className="text-gray-400 group-hover:text-gray-100 transition-colors duration-300">{step.description}</p>
            </motion.div>
          ))}
        </motion.div>
      </motion.div>
